...
**Zoom UUID**:
The Zoom identifier in the inbox filename between start and filetype. Each artifact has its own value, written to the matching column (`video_uuid`, `pdf_uuid`, `audio_uuid`, `transcript_uuid`, `chat_uuid`). Not part of the destination filename.
_Avoid_: UUID, event_id, zoom_meeting_id, zoom_uuid

**Inbox file**:
A recording dropped in the Drive inbox, named `{zoom_meeting_id}_{yyyy-MM-dd}_{HH-mm-ss}_{uuid}_{filetype}.{ext}`.
